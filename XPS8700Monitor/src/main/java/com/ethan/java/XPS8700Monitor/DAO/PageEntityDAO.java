package com.ethan.java.XPS8700Monitor.DAO;
...
public class PageEntityDAO {
	...
	private static PageEntityDAO instance = null;

	/**
	 * Constructor
	 */
	private PageEntityDAO() {
	 ...
	 */
	public static PageEntityDAO getInstance() {
		if (instance == null) {
			instance = new PageEntityDAO();
		}
		return instance;
	}
	 ...
	 */
	public void addObjectInstance(PageEntity instance) {
	 ...
	 */
	public List<PageEntity> getAllObjectInstances(String uri) {
		Filter uriFilter = new FilterPredicate("uri", FilterOperator.EQUAL, uri);
		Query query = new Query(OBJECT_INSTANCE)
				.setFilter(uriFilter)
				.addSort("timestamp", SortDirection.DESCENDING);
		...
		List<Entity> instances = datastoreService.prepare(query)
				.asList(FetchOptions.Builder.withDefaults());
		List<PageEntity> instanceList = new ArrayList<PageEntity>();
		for (Entity e : instances) {
			...
			instanceList.add(new PageEntity(uri, content, contentType,
	 ...
	 */
	public PageEntity getMostRecentObjectInstance(String uri) {
		Filter uriFilter = new FilterPredicate("uri", FilterOperator.EQUAL, uri);
		Query query = new Query(OBJECT_INSTANCE)
				.setFilter(uriFilter)
				.addSort("timestamp", SortDirection.DESCENDING);
		...
		int statusCode = ((Long) mostRecentInstance.getProperty("statusCode"))
				.intValue();
		return new PageEntity(uri, content, contentType, timestamp,
				statusCode);
	}
